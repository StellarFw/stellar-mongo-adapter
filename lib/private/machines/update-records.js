module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    (function findMatchingIdsMaybe(proceed) {
      ...
      mongoCollection
        .find(mongoWhere, projection)
        .toArray()
        .then((nativeResult) => proceed(undefined, _.pluck(nativeResult, pkColumnName)))
      ...
      mongoCollection
        .updateMany(secondaryMongoWhere, { $set: s3q.valuesToSet })
        .then(() => {
          ...
          if (!_.isUndefined(s3q.valuesToSet[pkColumnName])) {
            ...
            if (pkValsOfMatchingRecords.length === 0) {
              /* do nothing */
          ...
          mongoCollection
            ...
            .catch((err) => exits.error(err)); // </ mongoCollection.find() >
        })
        .catch((err) => {
