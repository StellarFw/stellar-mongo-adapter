  ...
  fn: function (inputs, exits) {
    ...
    var mongoSort = _.map(s3q.criteria.sort, function mapSort(s3qSortDirective) {
      ...
      assert(
        ...
        "At this point, the sort direction should always be ASC or DESC (capitalized).  If you are seeing this message, there is probably a bug somewhere in your version of Waterline core.",
      );
    ...
    try {
      assert(
        _.isNumber(s3q.criteria.limit),
        "At this point, the limit should always be a number, but instead it is `" +
          s3q.criteria.limit +
          "`.  If you are seeing this message, there is probably a bug somewhere in your version of Waterline core.",
      );
    ...
    if (s3q.criteria.select) {
      ...
      var projection = _.reduce(
        s3q.criteria.select,
        function reduceProjection(memo, colName) {
          memo[colName] = 1;
          return memo;
        },
        {},
    ...
    mongoDeferred
      ...
      .catch((err) => exits.error(err)); // </ mongoDeferred.toArray() >
  },
};
