  ...
  registerDatastore: function (dsConfig, physicalModelsReport, done) {
    ...
    try {
      ...
      switch (e.code) {
        case "E_BAD_CONFIG":
          return done(
            flaverr(e.code, new Error("Invalid configuration for datastore `" + datastoreName + "`:  " + e.message)),
          );
        default:
          return done(e);
      }
            ...
            new Error(
              numNotCompatible +
                ...
                _.reduce(
                  ...
                  function (memo, incompatibility, modelIdentity) {
    ...
    WET_MACHINES.destroyManager({ manager: dsEntry.manager }).switch({
      ...
      failed: function (report) {
        var err = new Error(
          "Datastore (`" +
            datastoreName +
            "`) could not be torn down because of a failure when attempting to destroy the connection manager.\n\n```\n" +
            report.error.stack +
            "\n```",
        );
        err.raw = report.error;
        if (report.meta) {
          err.meta = report.meta;
        }
        return done(err);
      },
      success: function (report) {
      ...
      function (key, next) {
        ...
        mongoCollection.createIndex(mongoSingleFieldIdxKeys, { unique: true }, function (err) {
          if (err && !_.isError(err)) {
            err = flaverr(
              { raw: err },
              new Error("Consistency violation: Expecting Error instance, but instead got: " + util.inspect(err)),
            );
            return next(err);
          } else if (err) {
            return next(err);
